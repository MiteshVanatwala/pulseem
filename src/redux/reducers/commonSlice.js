...
export const getCommonFeatures = createAsyncThunk(
	...
	async (req = null, thunkAPI) => {
		try {
			const settings = getCookie('accountSettings');
			if (
				!settings ||
				settings === '' ||
				(req && req.forceRequest === true) ||
				document.referrer.toLocaleLowerCase().indexOf('accountsmanage.aspx') >
					-1 ||
...
export const commonSlice = createSlice({
	...
	extraReducers: (builder) => {
		builder.addCase(getAuthorizedEmails.fulfilled, (state, { payload }) => {
			state.verifiedEmails = payload;
		});
		builder.addCase(getAuthorizeNumbers.fulfilled, (state, { payload }) => {
