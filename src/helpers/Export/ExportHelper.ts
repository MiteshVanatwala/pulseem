...
export const HandleExportData = async (
  ...
  return new Promise(async (resolve, reject) => {
    if (options.OrderItems === true) {
      try {
        finalExportData = await OrderItems(
          finalExportData,
          options.Order,
          options
        );
    ...
    if (options.ReplaceNull === true) {
      try {
        finalExportData = (await ReplaceNull(
          finalExportData,
          options.PropertyToReplace,
          options.PropertyDefaultReplaceValue
        )) as ExportData;
      } catch (error) {
        // Log({
    ...
    if (
      (options.ConvertStatusDescription === true ||
        options.ConvertStatusToString === true) &&
      options.Statuses !== null
    ) {
      try {
        finalExportData = await SwitchStatus(
          finalExportData,
          options.Statuses,
          options
        );
    ...
    if (options.BooleanToNumber === true) {
      try {
        finalExportData = (await BooleanToNumber(
          finalExportData,
          options.PropertyToReplace,
          options.IsBoolean
        )) as ExportData;
    ...
    if (options.DeleteProperties?.length > 0) {
      finalExportData = await DeletePropertyFromArrayObject(
        finalExportData,
        options.DeleteProperties
      );
    }
...
export async function OrderItems(
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    order.forEach((o: string | number) => {
      let value = data[i][o];
      if (
        options.ReplaceClientStatus === true &&
        o?.toString()?.toLowerCase() === "clientstatus"
      ) {
        value =
          data[i][o] === 0
            ? i18n.t("common.Subscribed")
            : i18n.t("common.Unsubscribed");
      }
      if (
        options.FormatDate === true &&
        DateOptions.filter((e) => {
          return e === o;
        })?.length > 0
      ) {
...
export async function SwitchStatus(
  ...
  data.forEach((o: Status) => {
    ...
    if (options.ConvertStatusDescription === true) {
      if (o.STATUS) {
        let status = statuses.find((s: { id: any }) => {
    ...
    if (options.ConvertStatusToString === true) {
      ...
      if (
        o.Attachments &&
        (o.Attachments === "No_Attachments" || o.Attachments === "")
      ) {
...
export async function ReplaceNull(
  obj: ExportData | any,
  property: string,
  val: string = ""
) {
...
export async function BooleanToNumber(
  obj: ExportData | any,
  property: string,
...
export async function DeletePropertyFromArrayObject(
  data: ExportData | any,
  properties: string[]
) {
  const retValData: any = [];
...
export const CreateFile = (data: any, type: string) => {
  return new Promise((resolve, reject) => {
    try {
      const f = new File([data], `${Date.now()}.${type}`, {
        type: `text/${type}`,
      });
      resolve(f);
    } catch (error) {
...
export const FlatObject = (obj: any = {}) =>
  Object.keys(obj || {}).reduce((o: any, cur) => {
