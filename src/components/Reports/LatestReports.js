          ...
          <Grid item lg={4} className={tabType !== "newsletter" ? classes.flexSpaceBetweenVertical : null}>
            {
              innerData.map((c, index) => {
                const campaignLink = `${actionURL}/CampaignStatistics.aspx?CampaignID=${c.CampaignID}` 
                return (
                    ...
                    <Grid item lg={12}>
                      <Box style={{ display: 'flex', alignItems: 'center' }}>
                        <BootstrapTooltip title={c.CampaignName} placement="top">
                          <Link target="_blank" rel="noopener" href={campaignLink} className={clsx(classes.dInlineBlock, classes.ellipsisText)} style={{ fontWeight: 'bold', maxWidth: 150 }}>
                            {c.CampaignName}
                          </Link>
                        </BootstrapTooltip>
