...
import { Typography, Divider, Grid, Button, Dialog as BaseDialog, Paper, Box } from '@material-ui/core'
...
export const Dialog = (options: Options) => {
    ...
    const renderExitButton = () => {
        return (
            <>
                {options.exit ? null : <Box
                    onClick={onExit}
                    className={clsx(
                        options.Classes.dialogExitButton,
                        {
    ...
    const renderTitleDefault = () => {
        return (
            <>
                <Typography className={clsx(options.reduceTitle ? options.Classes.reducedTitle : '', options.Classes.dialogTitle, windowSize !== 'xs' && windowSize !== 'sm' ? options.Classes.ellipsisText : null)}>
                    {options.Title}
                </Typography>
    ...
    const renderButtonsDefault = () => {
        return (
            options.ShowDefaultButtons && <Grid
                container
                spacing={4}
                className={clsx(options.Classes.dialogButtonsContainer, isRTL ? options.Classes.rowReverse : null)}>
                <Grid item>
                    <Button
                        variant='contained'
                        size='small'
    ...
    const renderIcon = () => {
        const alertIcon = <AlertIcon classes={options.Classes} />
        return (
            <Box
                className={clsx(
                    options.Classes.dialogIconContainer,
                    {
    ...
    const renderChildren = () => {
        return (
            <Box
                ...
                style={{
                    maxHeight: options.maxHeight ? options.maxHeight : windowSize !== 'sm' && windowSize !== 'xs' ? 'calc(65vh)' : 'calc(45vh)',
                    minWidth: windowSize !== 'xs' && windowSize !== 'sm' ? '330px' : '0px'
                }}>
                {options.Children}
            </Box>)
    }
    const renderContent = () => {
        return (
            <Box
                ...
                {renderChildren()}
                {options.RenderButtons ? options.RenderButtons() : renderButtonsDefault()}
            </Box>
        )
    }
    return (
        <BaseDialog
            style={options.Style}
            open={!!options.Open}
            className={clsx(options.Classes.dialogContainer, options.CustomContainerStyle !== '' ? options.CustomContainerStyle : null)}
            ...
            <Paper className={clsx(options.Classes.posRelative, options.PaperStyle, options.Classes.sidebar)}>
                {renderExitButton()}
                {renderContent()}
                {renderIcon()}
            </Paper>
