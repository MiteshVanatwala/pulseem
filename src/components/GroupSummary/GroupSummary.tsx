	...
	return (
      ...
      <Grid item xs={12} className={classes.pb15}>
        ...
        <Grid container>
          ...
          <Grid item xs={2} className={clsx(classes.paddingSides5)}>{summary?.TotalValidUploadedRecords || 0}</Grid>

          <Grid item xs={4}><Typography>{t("recipient.summary.totalInvalidOrEmptyAddresses")}</Typography></Grid>
          <Grid item xs={2} className={clsx(classes.paddingSides5)}>{summary?.TotalInvalidOrEmptyAddresses || 0}</Grid>
      ...
      <Grid item xs={12} className={classes.pb15}>
        ...
        <Grid container>
          ...
          <Grid item xs={2} className={clsx(classes.paddingSides5)}>{summary?.InvalidOrEmptyEmails || 0}</Grid>
          
          <Grid item xs={4}><Typography>{t("recipient.summary.duplicateEmailAddressesInFile")}</Typography></Grid>
          <Grid item xs={2} className={clsx(classes.paddingSides5)}>{summary?.DuplicateEmails || 0}</Grid>

          <Grid item xs={4}><Typography>{t("recipient.summary.totalEmailExistingInAccount")}</Typography></Grid>
          <Grid item xs={2} className={clsx(classes.paddingSides5)}>{summary?.ExistingEmails || 0}</Grid>
        </Grid>
      </Grid>
      ...
      <Grid item xs={12}>
        ...
        <Grid container>
          ...
          <Grid item xs={2} className={clsx(classes.paddingSides5)}>{summary?.InvalidOrEmptyCellphones || 0}</Grid>

          <Grid item xs={4}><Typography>{t("recipient.summary.duplicateCellphoneInFile")}</Typography></Grid>
          <Grid item xs={2} className={clsx(classes.paddingSides5)}>{summary?.DuplicateCellphones || 0}</Grid>

          <Grid item xs={4}><Typography>{t("recipient.summary.totalCellularExistingInAccount")}</Typography></Grid>
          <Grid item xs={2} className={clsx(classes.paddingSides5)}>{summary?.ExistingCellphones || 0}</Grid>
        </Grid>
      </Grid>
    </Grid>
	);
