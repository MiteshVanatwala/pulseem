...
import { 
  Box, Button, Grid, ListItem, ListItemAvatar, Avatar, ListItemText, Paper, 
  Tab, Tabs, Typography, Popper, Fade, List, Collapse, Divider, IconButton, 
  CircularProgress 
...
import { 
  getRecipientsReport, getPackagesDetails, getLastCampaignReport, getTips, getShortcuts, setShortcuts
} from '../redux/reducers/dashboardSlice';

const DashboardScreen=({classes}) => {
  const {language,windowSize,isRTL}=useSelector(state => state.core);
  const {username}=useSelector(state => state.user);
  const {recipientsReport,lastCampaignReport,packagesDetails,tips,shortcuts,recipientsReportError,
    lastCampaignReportError,packagesDetailsError,tipsError,shortCutsError }=useSelector(state => state.dashboard);
  const {smsData}=useState('0');
  ...
  //       shortcutRef.current.style.position='fixed'
  //       shortcutRef.current.style.top='0'
  //       shortcutRef.current.style.paddingBottom='47px'
  //     } else {
  //       shortcutRef.current.style.position='unset'
  //       shortcutRef.current.style.top='47'
  //       shortcutRef.current.style.paddingBottom='0'
  //     }
  //   }
  // }
                    ...
                    <ListItemText
                      ...
                      secondary={
                        <Box className={classes.dFlex}>
                          <Typography variant={'body2'}>{reportData.total.toLocaleString()}</Typography>
                          <Typography variant={'body2'} className={classes.ml5}>{date}</Typography>
                        </Box>}
                    />
                  </ListItem>
                </Grid>
            ...
            <ListItemText
              secondary={
                <Box className={classes.lastReportItemText}>
                  <Typography 
                    className={clsx(classes.dashboardTitle, classes.dInline, classes.pe10)}>
