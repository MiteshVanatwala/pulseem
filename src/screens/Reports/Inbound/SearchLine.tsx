...
const SearchLine = ({
  ...
  const [searchRequest, setSearchRequest] =
    useState<RequestObject>(DEFAULT_REQUES);
  const [isFromDatePickerOpen, setIsFromDatePickerOpen] = useState<
    boolean | undefined
  >(false);
  const [isToDatePickerOpen, setIsToDatePickerOpen] = useState<
    boolean | undefined
  >(false);
  const [advanceSearch, setAdvanceSearch] = useState<boolean | undefined>(
    false
  );
      ...
      <>
        <Grid item>
          <TextField
            ...
            value={searchRequest.FromNumber}
            onChange={(e) =>
              setSearchRequest({ ...searchRequest, FromNumber: e.target.value })
            }
            className={clsx(classes.textField, classes.minWidth252)}
            placeholder={t("common.FrmNumber")}
          />
        ...
        <Grid item>
          <TextField
            ...
            onChange={(e) =>
              setSearchRequest({ ...searchRequest, ToNumber: e.target.value })
            }
            className={clsx(classes.textField, classes.minWidth252)}
        ...
        {windowSize !== "xs" && (
          <Link
            ...
            className={clsx(
              ...
              classes.dBlock,
              classes.mt1,
              advanceSearch && windowSize === "lg" ? classes.mb15 : null
            )}
          >
            <>
              {t(
                !advanceSearch
                  ? "report.AdvanceSearch"
                  : "report.closeAdvanceSearch"
              )}
            </>
          </Link>
        )}
