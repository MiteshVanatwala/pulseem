        ...
        <Box style={{ padding: 25, maxWidth: 900, margin: '0 auto' }}>
          <Grid container spacing={1}>
            <Grid item xs={4}>
              ...
              <Typography title={webForm.PageName} className={classes.ellipsisText}>
                {webForm.PageName}
              </Typography>
            </Grid>
            <Grid item xs={4}>
            ...
            <Grid item xs={12}><Divider style={{ margin: '10px 0px' }} /></Grid>
            ...
            {subAccountAllGroups?.length > 0 && (
              <Grid item xs={4}>
                ...
                {webForm.SelectedGroupList?.map((groupId: any) => {
                  const groupFound = subAccountAllGroups?.find((g: any) => g?.GroupID === parseInt(groupId));
                  return (
                    <Typography key={groupId} title={groupFound?.GroupName} className={classes.ellipsisText}>
            ...
            <Grid item xs={4}>
              ...
              <Typography
                title={webForm.SubscriptionsLimit > 0 ? webForm.SubscriptionsLimit : t('common.disabled')}
                className={classes.ellipsisText}
              >
                {webForm.SubscriptionsLimit > 0 ? webForm.SubscriptionsLimit : t('common.disabled')}
              </Typography>
            </Grid>
            ...
            <Grid item xs={4}>
              ...
              <Typography
                title={webForm.OfflineDate ? moment(webForm.OfflineDate).format(DateFormats.DATE_ONLY) : t('common.notSet')}
              >
                {webForm.OfflineDate ? moment(webForm.OfflineDate).format(DateFormats.DATE_ONLY) : t('common.notSet')}
              </Typography>
            </Grid>

            <Grid item xs={4}>
              <Typography className={classes.bold}>{t('landingPages.redirectURLWhenOffline')}</Typography>
              <Typography title={webForm.OfflineUrl} className={classes.ellipsisText}>
                {webForm.OfflineUrl || t('common.notSet')}
              </Typography>
            </Grid>

            <Grid item xs={4}>
              <Typography className={classes.bold}>{t('landingPages.reportLeadsToEmails')}</Typography>
              {webForm?.EmailsToReport?.split(',')?.length > 0 ? (
                webForm?.EmailsToReport.split(',')?.map((email: string, index: number) => (
                  <Typography key={index} className={classes.ellipsisText}>
                    {email}
                  </Typography>
                ))
              ) : (
