    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const clientForm = {
                ...
                Email: new BeeFormModel(ElementTypes.text, t('common.email'), false, { ...fieldDirection, maxlength: '100', required: 'true', placeholder: t('landingPages.placeholder.email') }),
                Cellphone: new BeeFormModel(ElementTypes.text, t('common.cellphone'), false, { ...fieldDirection, pattern: '\\d*', required: 'true', placeholder: t('landingPages.placeholder.cellphone') }),
                Zip: new BeeFormModel(ElementTypes.text, t('common.zip'), true, { ...fieldDirection, maxlength: '50', pattern: '\\d*', placeholder: t('landingPages.placeholder.zip') }),
                ...
                Telephone: new BeeFormModel(ElementTypes.text, t('common.telephone'), true, { ...fieldDirection, size: '100', pattern: '\\d*', placeholder: t('landingPages.placeholder.telephone') }),
                BirthDate: new BeeFormModel(ElementTypes.text, t('common.birth_date'), true, { ...fieldDirection, placeholder: t('landingPages.placeholder.birth_date') }, 'frm-1 bee-date'),
                ReminderDate: new BeeFormModel(ElementTypes.text, t('common.reminder_date'), true, { ...fieldDirection, placeholder: t('landingPages.placeholder.reminder_date') }, 'frm-1 bee-date'),
            ...
            if (extraFields && Object.keys(extraFields)?.length > 0) {
                Object.keys(extraFields).forEach((key) => {
                    ...
                    if (val !== '') {
                        clientForm[key] = new BeeFormModel(ElementTypes.text, val, true, { ...fieldDirection, placeholder: t('landingPages.placeholder.typeHere') + ' ' + val, name: key }, key?.toLowerCase()?.indexOf('date') > -1 && 'frm-1 bee-date');
                    }
                });
            }
            resolve(clientForm);
