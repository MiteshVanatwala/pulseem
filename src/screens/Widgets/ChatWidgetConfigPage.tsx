...
import { Grid, Typography, Box, Tabs, Tab, TextField, Button, IconButton, Chip, CircularProgress, Dialog, DialogTitle, DialogContent, DialogActions } from '@material-ui/core';
...
const ChatWidgetConfigContent = ({ classes, initialConfig, initialWidgetId, initialStatus, domain }: ChatWidgetConfigContentProps) => {
  ...
  return (
    ...
    >
      <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={4} mt={2}>
        <Box>
          <Button
            ...
            size="small"
            style={{ textTransform: 'none', color: '#6b7280', marginBottom: 8, marginInlineStart: -8 }}
          >
      ...
      <Dialog open={!!tierBlockedFeature} onClose={() => setTierBlockedFeature(null)} maxWidth="xs" fullWidth>
        <DialogTitle>{t('billing.tier.permission', 'Upgrade required')}</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          <Button onClick={() => setTierBlockedFeature(null)} style={{ textTransform: 'none' }}>
            {t('common.cancel', 'Cancel')}
          </Button>
          <Button
            variant="contained"
            disableElevation
            style={{ backgroundColor: ACCENT, color: '#fff', textTransform: 'none', fontWeight: 600 }}
            onClick={() => { window.location.href = `${sitePrefix}BillingSettings`; }}
          >
