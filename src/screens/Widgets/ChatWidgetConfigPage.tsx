...
const ChatWidgetConfigContent = ({ classes, initialConfig, initialWidgetId, initialStatus }: ChatWidgetConfigContentProps) => {
  ...
  const handleStatusChange = async (nextStatus: 'active' | 'paused') => {
    ...
    try {
      const result = await setWidgetStatus(nextStatus);
      setStatus(result.status as WidgetStatus);
    } catch (err) {
...
const ChatWidgetConfigPage = ({ classes }: { classes?: any }) => {
  const [loadState, setLoadState] = useState<LoadState>({ loading: true, config: initialWidgetConfig });
