...
const FeedbackTab: React.FC<FeedbackTabProps> = ({ config, onChange }) => {
  ...
  // Reuses the existing account-wide sub-user list (same endpoint the SubUsers
  // management page calls) rather than a widget-specific lookup.
  useEffect(() => {
    let cancelled = false;
    PulseemReactInstance
      .post<PulseemResponse>('SubUser/GetAllUsers', { PageNumber: 1, PageSize: 1000, SearchTerm: '' })
      .then((response) => {
        if (cancelled) return;
        if (response.data.StatusCode === 201) {
          const users: SubUserModel[] = response.data.Data?.Users || [];
          setAgents(
            users
              .filter((u) => !u.IsDeleted)
              .map((u) => ({
                id: u.ID,
                name: (!u.FirstName && !u.LastName) ? u.Email : `${u.FirstName || ''} ${u.LastName || ''}`.trim(),
              }))
          );
        } else if (response.data.StatusCode !== 404) {
          console.error('Failed to load agents', response.data.Message);
        }
      })
          ...
          >
            <FormControl fullWidth variant="outlined" size="small">
              ...
              >
                ...
                {agents.map((agent) => (
                  <MenuItem key={agent.id} value={String(agent.id)}>{agent.name}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </SettingRow>
